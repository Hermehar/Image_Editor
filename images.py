...
def redscreen(main_filename, back_filename):                                                     """used for redscreen"""
    ...
    for pixel in image:
        ...
        if pixel.red >= average * INTENSITY_THRESHOLD:                                    """it helps to merge two different images"""
            x = pixel.x
            y = pixel.y
...
def darker(image):                                                                      """gets you the darker version of the image"""
    for pixel in image:
        ...
        pixel.green = pixel.green // 2                                                """RGB pixel values are to be decreased"""
        pixel.blue = pixel.blue // 2

def red_channel(filename):                                                         """converts the image into red color"""
    for pixel in filename:
        pixel.green = 0
        pixel.blue = 0                                                             """put the values of green and blue pixels to 0"""
    return filename
